...
export const CONFIG_TOOLTIPS: Record<string, string> = {
  ...
  // Latency
  DDR_R_LATENCY: 'DDR read latency in ns',
  DDR_R_LATENCY_VAR: 'DDR read latency variance',
  DDR_W_LATENCY: 'DDR write latency in ns',
  L2M_R_LATENCY: 'L2M read latency in ns',
  L2M_W_LATENCY: 'L2M write latency in ns',
  ...
  CHANNEL_SELECT_STRATEGY: 'Channel selection strategy: coord_based, node_id_based, burst_interleave, ip_type_id_based, round_robin',
  CHANNEL_SELECT_PARAMS: 'Channel selection params: basis(source/destination), dimension(x/y), granularity(packet/flit/request)',
  // D2D Config (DCIN模式)
