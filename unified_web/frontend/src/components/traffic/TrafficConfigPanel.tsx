...
import { computeStaticBandwidth, getDCINConfigs } from '../../api/staticBandwidth'
...
const TrafficConfigPanel: React.FC<TrafficConfigPanelProps> = ({ topology, mode, mountsVersion, onBandwidthComputed }) => {
  ...
  const [dcinConfigs, setDCINConfigs] = useState<Array<{ filename: string; num_dies: number; connections: number }>>([])
  const [selectedDCINConfig, setSelectedDCINConfig] = useState<string>('')
  ...
  // 加载DCIN配置文件列表
  const loadDCINConfigs = async () => {
    try {
      const data = await getDCINConfigs()
      setDCINConfigs(data.configs)
      if (data.configs.length > 0 && !selectedDCINConfig) {
        setSelectedDCINConfig(data.configs[0].filename)
      }
    } catch (error) {
      console.error('加载DCIN配置失败:', error)
    }
  }
  ...
  const handleComputeBandwidth = async () => {
    ...
    try {
      const result = await computeStaticBandwidth({
        ...
        dcin_config_file: mode === 'dcin' ? selectedDCINConfig : undefined
      })

      if (result.success) {
        // 根据模式显示不同的统计信息
        if (result.mode === 'dcin') {
          // DCIN模式：显示所有Die中的最大带宽
    ...
    <Card
      title={
        <Space>
          ...
          <Tag color={mode === 'kcin' ? 'blue' : 'orange'}>{mode.toUpperCase()} 模式</Tag>
        </Space>
      }
      extra={
        <Space>
    ...
    >
          ...
          expandedRowRender: (record) => (
            <div style={{ padding: '8px 16px', backgroundColor: '#f9f9f9' }}>
              <Space direction="vertical" size="small" style={{ width: '100%' }}>
                ...
                {mode === 'dcin' && record.target_dies && record.target_dies.size > 0 ? (
                  <div>
        ...
        <Space>
          ...
          {mode === 'dcin' && (
            <>
              <span>DCIN配置:</span>
              <Select
                value={selectedDCINConfig}
                onChange={setSelectedDCINConfig}
                style={{ width: 180 }}
                placeholder="选择DCIN配置"
              >
                {dcinConfigs.map(config => (
          ...
          <Button
            ...
            loading={computing}
            disabled={configs.length === 0 || (mode === 'dcin' && !selectedDCINConfig)}
          >
            计算静态链路带宽
          </Button>
        </Space>
      ...
      <Space direction="vertical" style={{ marginTop: 16, width: '100%', alignItems: 'center' }}>
        ...
        <Space size="large" style={{ width: 600 }}>
          ...
          >
            按源IP拆分流量文件
            <Tooltip title="生成后自动将流量文件按源IP拆分成多个独立文件，存放在以文件名命名的文件夹中。文件名格式：KCIN模式为 master_p{ip_index}_x{x}_y{y}.txt，DCIN模式为 master_d{die_id}_p{ip_index}_x{x}_y{y}.txt">
