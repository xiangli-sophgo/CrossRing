...
from optimal_placement import create_adjacency_matrix, find_shortest_paths
...
class Simulation:
    ...
    def initial(self):
        ...
        self.send_read_flits_num = 0
        self.send_write_flits_num = 0
        ...
        if len(self.node.sn_wdb_recv[self.sn_type][in_pos]) > 0:
            ...
            if len(self.node.sn_wdb[self.sn_type][in_pos][packet_id]) == 0:
                ...
                if self.node.sn_wdb_count[self.sn_type][in_pos] > 0 and self.node.sn_req_wait["write"][self.sn_type][in_pos]:
                    new_req = self.node.sn_req_wait["write"][self.sn_type][in_pos].pop(
                        0)
    ...
    def log_summary(self):
        print(
            f"T: {self.cycle // self.config.network_frequency}, Req_cnt: {self.req_count} Inject_Req: {self.req_num}, rsp: {self.rsp_num},"
                            ...
                            if i == 0:
                                ...
                                if flit.is_last_flit:
                                    ...
                                    if self.node.sn_req_wait["read"][self.sn_type][ip_pos]:
                                        ...
                                        new_req = self.node.sn_req_wait["read"][self.sn_type][ip_pos].pop(
                                            0)
                                        new_req.sn_tracker_type = req.sn_tracker_type
                                        new_req.req_attr = "old"
                                        self.node.sn_tracker[self.sn_type][ip_pos].append(
                                            new_req)
                                        self.node.sn_tracker_count[self.sn_type][req.sn_tracker_type][ip_pos] -= 1
                                        self.create_rsp(new_req, "positive")
                            else:
                                ...
                                if flit.is_last_flit:
                                    ...
                                    self.node.rn_tracker["write"][self.rn_type][ip_pos].remove(
                                        req)
    ...
    def _update_transfer_stations(self, network, pos, next_pos, index):
        """更新transfer stations"""
        if index == 0:
            network.transfer_stations["up"][(pos, next_pos)].popleft()
        elif index == 1:
            network.transfer_stations["left"][(pos, next_pos)].popleft()
        elif index == 2:
            network.transfer_stations["right"][(pos, next_pos)].popleft()
        ...
        if req.req_type == "read":
            if req.req_attr == "new":
                ...
                #     self.create_read_packet(req)
                    ...
                    self.node.sn_req_wait[req.req_type][self.sn_type][in_pos].append(
                        req)
            else:
                self.create_read_packet(req)
    ...
    def _update_flit_state(self, network, ts_key, pos, next_pos, target_node, direction):
        flit = network.transfer_stations[ts_key][(pos, next_pos)].popleft()
    ...
    def _handle_response(self, rsp, in_pos):
        ...
        if rsp.req_type == "read":
            ...
            if rsp.rsp_type == "negative":
                ...
                self.node.rn_wdb_send[self.rn_type][in_pos].append(
                    rsp.packet_id)
        ...
        if self.cycle % (self.config.seats_per_link * (self.config.cols - 1) * 2 + 4) == 0:
            for i, j in network.links:
                if i - j == 1 or (i == j and (i % self.config.cols == self.config.cols - 1 and (i // self.config.cols) % 2 != 0)):
                    ...
                    i == j
                    and i in range(self.config.num_nodes - self.config.cols * 2, self.config.cols + self.config.num_nodes - self.config.cols * 2)
                ):
    ...
    def calculate_predicted_duration(self, flit):
        ...
        for i in range(1, len(flit.path)):
            duration += 2 if flit.path[i] - flit.path[i - 1] == - \
                self.config.cols else self.config.seats_per_link
    ...
    def evaluate_performance(self, network):
        ...
        for source in self.config.sdma_send_positions:
            network.throughput["gdma"][source][0] = (
                ...
                * self.config.network_frequency
                / (network.throughput["gdma"][source][3] - network.throughput["gdma"][source][2])
            )
        ...
        for source in self.config.ddr_send_positions:
            network.throughput["l2m"][source][0] = (
                network.throughput["l2m"][source][1]
                * self.config.flit_size
                * self.config.network_frequency
                / (network.throughput["l2m"][source][3] - network.throughput["l2m"][source][2])
            )
                ...
                if throughput_data[key]["last_recv"] > 0:
                    if key == "sdma":
                        throughput_data[key][f"{key}_throughput"] = (
                            ...
                            * 256
                            / (throughput_data["sdma"]["last_recv"] - throughput_data["ddr"]["first_send"])
                    ...
                    #     )
                        ...
                        throughput_data[key][f"{key}_throughput"] = (
                            throughput_data[key]["recv_num"]
                            * 256
                            / (throughput_data["l2m"]["last_recv"] - throughput_data["gdma"]["first_send"])
                            / throughput_data[key]["num"]
                        )
                    elif key == "gdma":
                        throughput_data[key][f"{key}_throughput"] = (
                            throughput_data[key]["recv_num"]
                            * 256
                            / (throughput_data["gdma"]["last_recv"] - throughput_data["l2m"]["first_send"])
                            / throughput_data[key]["num"]
                        )
            begin_id = end_id
    ...
    def node_change(self, node, is_source=True):
        if is_source:
            ...
            if self.topo_type in ["5x4", "4x5"]:
                return (
                    self.config.gdma_send_positions[node] - self.config.cols
                    if node < 16
                    else self.config.sdma_send_positions[node % 16] - self.config.cols
                )
            elif self.topo_type == "6x5":
...
def main():
    ...
    result_save_path = r"../Result/cross ring/REQ_RSP/large/sn_tracker-v1.0/"
    ...
    sim = Simulation(
        config_path=config_path, topo_type=topo_type, traffic_file_path=traffic_file_path, file_name=file_name, result_save_path=result_save_path
    )
