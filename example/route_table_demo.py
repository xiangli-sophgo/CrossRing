...
def demo_basic_route_table():
    ...
    # 添加几条路由
    print("添加路由条目...")
    route_table.add_route_with_validation(destination=1, next_hop=1, path=[1], metric=1.0, priority=1, direction="DIRECT")

    route_table.add_route_with_validation(destination=2, next_hop=1, path=[1, 2], metric=2.0, priority=1, direction="VIA_1")

    route_table.add_route_with_validation(destination=3, next_hop=1, path=[1, 2, 3], metric=3.0, priority=1, direction="VIA_1_2")
...
def demo_ring_topology_routing():
    ...
    if route_table:
        for dest in [2, 4, 6]:
            ...
            # 带拥塞上下文的查询
            context = {"load_balancing": True, "congestion_threshold": 0.6, f"link_congestion_{route.next_hop}": 0.8, "direction_bias_CW": 0.2, "direction_bias_CCW": 0.1}  # 模拟拥塞
...
def demo_route_table_persistence():
    ...
    for i in range(1, 5):
        route_table.add_route_with_validation(destination=i, next_hop=min(i, 2), path=list(range(min(i, 2), i + 1)) if i > 2 else [i], metric=float(i), priority=1, direction=f"TO_{i}")
...
def demo_dynamic_routing_with_congestion():
    ...
    # 主路由：直接路径
    route_table.add_route_with_validation(destination=destination, next_hop=1, path=[1, 2, 3], metric=3.0, priority=1, direction="CW")

    # 备用路由：反向路径
    route_table.add_route_with_validation(destination=destination, next_hop=5, path=[5, 4, 3], metric=3.0, priority=2, direction="CCW")
    ...
    # 场景2：主路由拥塞
    context2 = {"congestion_threshold": 0.8, "link_congestion_1": 0.9, "load_balancing": True}  # 主路由拥塞
    ...
    # 场景3：方向偏好
    context3 = {"load_balancing": True, "direction_bias_CW": 0.3, "direction_bias_CCW": 0.1}
            ...
            for dest in range(num_nodes):
                if dest != node_id:
                    ...
                    route_table.add_route_with_validation(
                        destination=dest, next_hop=next_hop, path=[next_hop, dest] if dest != next_hop else [dest], metric=abs(dest - node_id), priority=1, direction="FORWARD"
                    )
