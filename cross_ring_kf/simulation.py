...
class Simulation:
    ...
    def update_intervals(self, flit, merged_intervals, latency, file, req_type):
        ...
        print(
            f"{flit.req_departure_cycle // self.config.network_frequency},{flit.source_original},{flit.source_type},{flit.destination_original},{flit.destination_type},"
...
def main():
    ...
    result_save_path = r"../Result/cross ring/v8-32/3M"
    ...
    # sim.end_time = 10000
    sim.config.rn_read_tracker_ostd = 60
...
def find_optimal_parameters():
    ...
    # 创建结果保存路径
    result_root_save_path = r"../Result/cross ring/optimal/write_after_read/ro_and_shared/"
    ...
    for parm1 in range(parm1_start, parm1_end + 1, parm1_step):  # 使用 parm1_end + 1 以包含结束值
        for parm2 in range(parm2_start, parm2_end + 1, parm2_step):  # 使用 parm2_end + 1 以包含结束值
            ...
            sim.config.rn_rdb_size = sim.config.rn_read_tracker_ostd * 4
            sim.config.rn_wdb_size = sim.config.rn_write_tracker_ostd * 4
            sim.config.sn_wdb_size = 1000
            sim.config.ro_tracker_ostd = 1000
            sim.config.share_tracker_ostd = 1000

            # sim.config.update_config()
            sim.initial()
            sim.end_time = 60000
            sim.print_interval = 10000
            print(f"rn_read_tracker_ostd: {parm1}, rn_write_tracker_ostd: {parm2}")
