...
def find_optimal_parameters():
    ...
    # 定义参数范围
    parm1_start, parm1_end, parm1_step = (2, 2, 1)
    parm2_start, parm2_end, parm2_step = (2, 2, 1)
    ...
    for parm1 in range(parm1_start, parm1_end + 1, parm1_step):
        for parm2 in range(parm2_start, parm2_end + 1, parm2_step):
            ...
            sim_vars = vars(sim)
            results = {key[1:]: value for key, value in sim_vars.items() if key.startswith("_") and not callable(value)}

            config = {key: value for key, value in vars(sim.config).items()}
