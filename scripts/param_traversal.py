...
FIFO_PARAMS = {
    ...
    "RB_IN_FIFO_DEPTH": {"range": [2, 20], "default": 16},
    # "EQ_IN_FIFO_DEPTH": {"range": [2, 12], "default": 16},
}

# ETag参数配置（包含约束关系）
ETAG_PARAMS = {
    "TL_Etag_T2_UE_MAX": {"range": [1, 19], "default": 8, "related_fifo": "RB_IN_FIFO_DEPTH", "constraint": "less_than_fifo"},
    "TL_Etag_T1_UE_MAX": {"range": [2, 19], "default": 15, "related_fifo": "RB_IN_FIFO_DEPTH", "constraint": "less_than_fifo_and_greater_than_t2", "corresponding_t2": "TL_Etag_T2_UE_MAX"},
    "TR_Etag_T2_UE_MAX": {"range": [1, 19], "default": 12, "related_fifo": "RB_IN_FIFO_DEPTH", "constraint": "less_than_fifo"},
...
def save_optimization_results_to_csv(result_data, output_csv, db_manager=None, experiment_id=None, save_csv=True):
    """保存结果到CSV文件和/或数据库"""
    ...
    with save_optimization_results_to_csv._lock:
        try:
            ...
            if db_manager is not None and experiment_id is not None:
                try:
                    # 提取配置参数（FIFO/ETag 参数）
                    config_params = {k: v for k, v in result_data.items() if k in ALL_PARAMS.keys()}
                    # 提取性能指标
                    performance = result_data.get("带宽_DDR_混合", result_data.get("mixed_avg_weighted_bw", result_data.get("optimization_performance", 0)))
                    # 其他数据作为详细结果
                    result_details = {k: v for k, v in result_data.items() if k not in config_params}
...
class ParamTraversalRunner:
    ...
    def __init__(self, config_path: str, topo_type: str, traffic_files: List[str], traffic_weights: List[float], traffic_path: str = "../traffic/0617/", experiment_name: str = None, save_csv: bool = True):
        ...
        if experiment_name:
            try:
                self.db_manager = ResultManager()
                self.experiment_id = self.db_manager.create_experiment(
                    ...
                    with ProcessPoolExecutor(max_workers=n_jobs) as executor:
                        ...
                        for future in as_completed(future_to_combination):
                            ...
                            try:
                                ...
                                save_optimization_results_to_csv(error_result, self.csv_output_path, self.db_manager, self.experiment_id, self.save_csv)
    ...
    def _generate_summary_report(self):
        ...
        report = []
        report.append("=" * 60)
        report.append("FIFO和ETag参数全遍历报告")
...
def main():
    """主函数"""
    print("=" * 60)
    print("FIFO和ETag参数全遍历")
    print("=" * 60)

    # 获取实验名称（用于数据库记录）
    experiment_name = input("请输入实验名称（留空则自动生成）: ").strip()
    if not experiment_name:
        experiment_name = f"exp_{datetime.now().strftime('%m%d_%H%M')}"
    ...
    try:
        # 运行全遍历
        runner.run_all_exhaustive_searches()

        # 生成可视化
        runner.visualize_parameter_results()

        # 保存结果
        runner.save_results()

        # 更新实验状态为完成
        if runner.db_manager and runner.experiment_id:
            runner.db_manager.update_experiment_status(runner.experiment_id, "completed")
            print(f"实验状态已更新为: completed")
        ...
        runner.save_results()
        if runner.db_manager and runner.experiment_id:
            runner.db_manager.update_experiment_status(runner.experiment_id, "interrupted")
    except Exception as e:
        print(f"\n遍历过程出错: {e}")
        traceback.print_exc()
        runner.save_results()
        if runner.db_manager and runner.experiment_id:
            runner.db_manager.update_experiment_status(runner.experiment_id, "failed")
