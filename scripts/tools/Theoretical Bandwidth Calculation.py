...
class Event:
    ...
    def __init__(self, time, typ, dma_id=None, port_id=None, cmd=None,
                 flit_idx=None, req_id=None):
        self.time = time
        self.type = typ
        self.dma_id = dma_id
...
class NoC_Simulator:
    def __init__(
        self,
        # ------ DMA & MEM 规模 ------
        n_gdma=1, n_sdma=1, n_ddr=1, n_l2m=1,
        # ------ RN outstanding ------
        RN_ostd_rd_gdma=4, RN_ostd_wr_gdma=4,
        RN_ostd_rd_sdma=4, RN_ostd_wr_sdma=4,
        # ------ SN outstanding ------
        SN_ostd_rd_ddr=4, SN_ostd_wr_ddr=4,
        SN_ostd_rd_l2m=4, SN_ostd_wr_l2m=4,
        # ------ 延迟 / 带宽 ------
        ddr_latency=None, l2m_latency=None,
        latency_var=(0, 0),
        path_delay=10,
        num_cmds=200,
        burst_length=4,
        ...
        self.tokens = [self.ddr_rate if t == MemType.DDR else self.l2m_rate
    ...
    def all_dma_free(self):
        return all(d.tracker_rd.count == 0 and d.tracker_wr.count == 0
                   for d in self.RN)
    ...
    def run(self):
        while self.events:
            ...
            if ev.type == Event.RN_CMD:               self.on_rn_cmd(ev)
            elif ev.type == Event.SN_RECEIVE_CMD:     self.handle_sn_receive(ev)
            elif ev.type == Event.SN_DONE:            self.handle_sn_done(ev)
            elif ev.type == Event.RN_DATA:            self.handle_rn_data(ev)
            elif ev.type == Event.RN_ACK:             self.handle_rn_ack(ev)
            elif ev.type == Event.RN_SEND:            self.handle_rn_send(ev)
            elif ev.type == Event.SN_DATA:            self.handle_sn_data(ev)
            elif ev.type == Event.SN_RELEASE_CREDIT:  self.SN[ev.port_id].tracker_wr.dec()
    ...
    def choose_req(self, dma_id):
        ...
        while True:
            ...
            if dma_str != need:
                continue
            # 端口候选
            ports = [i for i, t in enumerate(self.mem_types)
                     if (mem_str == "ddr" and t == MemType.DDR) or
                        (mem_str == "l2m" and t == MemType.L2M)]
            return random.choice(ports), cmd   # 不轮询，保持随机
    ...
    def on_rn_cmd(self, ev: Event):
        ...
        if not self._gap_allows(ev.dma_id, mem_str, cmd):
            # 达到硬限制：稍后重试
            self.schedule(Event(self.current_time + 1.0, Event.RN_CMD,
                                dma_id=ev.dma_id))
            return
        ...
        if tr_dma.can_accept() and tr_mem.can_accept():
            ...
            self.requests[req_id] = Request(req_id, ev.dma_id, port_id, cmd,
                                            self.current_time)

            self.schedule(Event(self.current_time + self.path_delay,
                                Event.SN_RECEIVE_CMD,
                                dma_id=ev.dma_id, port_id=port_id, cmd=cmd,
                                req_id=req_id))
    ...
    def handle_sn_receive(self, ev: Event):
        ...
        self.schedule(Event(t_done, Event.SN_DONE, dma_id=ev.dma_id,
                            port_id=port, cmd=ev.cmd, req_id=ev.req_id))
    ...
    def handle_sn_done(self, ev: Event):
        ...
        if cmd == "R":
            ...
            self.schedule(Event(ev.time + self.release_latency,
                                Event.SN_RELEASE_CREDIT,
                                port_id=port, cmd="W"))
        ...
        for i in range(send_cnt):
            ...
            self.schedule(Event(t_send + self.path_delay, etype,
                                dma_id=ev.dma_id, port_id=port, cmd=cmd,
                                flit_idx=(i if cmd == "R" else None),
                                req_id=ev.req_id))
    ...
    def handle_rn_ack(self, ev: Event):
        ...
        self.schedule(Event(ev.time, Event.RN_SEND, dma_id=ev.dma_id,
                            port_id=ev.port_id, cmd="W", req_id=ev.req_id))
    ...
    def handle_rn_send(self, ev: Event):
        ...
        for i in range(self.burst_length):
            ...
            t0 += self.ip_gap
            self.schedule(Event(send_time + self.path_delay, Event.SN_DATA,
                                dma_id=dma_id, port_id=port, cmd="W",
                                flit_idx=i, req_id=ev.req_id))
    ...
    def report(self):
        ...
        for k, data in groups.items():
            ...
            mask = ts <= t85
            plt.plot(ts[mask] / 1000, bw[mask], drawstyle="steps-post",
                     label=f"{k[0].upper()}-{k[1]}")
            total_bw += bw[-1]
        ...
        for rule in self.gap_rules:
            ...
            print(f"[GAP] {key}: read={cnt['read']} write={cnt['write']} "
                  f"diff={diff} limit={rule['limit']}")
        ...
        if self.save_req_trace:
            with open("sim_trace.csv", "w", newline="") as f:
                wr = csv.writer(f)
                wr.writerow(["req", "dma", "type", "mem_port", "mem_type", "cmd",
                             "cmd_t", "sn_recv_t", "sn_sends", "rn_sends",
                             "read_recv", "write_recv"])
