...
from src.kcin.v1.components import Network, IPInterface
...
class BaseModel(StatsMixin, DataflowMixin):
    ...
    def _get_positions_list(self, network_type):
        """根据网络类型返回位置列表"""
        if network_type == "req":
            return self.rn_positions_list
        elif network_type == "rsp":
            return self.sn_positions_list
        return self.flit_positions_list
    ...
    def initial(self):
        ...
        req_bothside = self.config.ETAG_BOTHSIDE_UPGRADE or (self.config.ORDERING_PRESERVATION_MODE in [2, 3] and "REQ" in self.config.IN_ORDER_PACKET_CATEGORIES)
        rsp_bothside = self.config.ETAG_BOTHSIDE_UPGRADE or (self.config.ORDERING_PRESERVATION_MODE in [2, 3] and "RSP" in self.config.IN_ORDER_PACKET_CATEGORIES)
        data_bothside = self.config.ETAG_BOTHSIDE_UPGRADE or (self.config.ORDERING_PRESERVATION_MODE in [2, 3] and "DATA" in self.config.IN_ORDER_PACKET_CATEGORIES)
        for network in self.req_networks:
            network.ETAG_BOTHSIDE_UPGRADE = req_bothside
        for network in self.rsp_networks:
            network.ETAG_BOTHSIDE_UPGRADE = rsp_bothside
        for network in self.data_networks:
            network.ETAG_BOTHSIDE_UPGRADE = data_bothside
    ...
    def step(self):
        ...
        if self.cycle % self.config.NETWORK_SCALE == 0:
            ...
            # Network arbitration and movement - 通用多通道循环
