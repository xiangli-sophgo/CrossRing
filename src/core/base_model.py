...
class BaseModel:
    ...
    def initial(self):
        ...
        self.rn_send_num = 0
        self.negative_rsp_num, self.positive_rsp_num = 0, 0
                ...
                if network.links_tag[link][-1]:
                    if flit_l.destination == next_pos:
                        ...
                        if network.links_tag[link][-1] == [
                            next_pos,
                            direction,
                        ] and network.config.EQ_FIFO_depth - len(
                            eject_queue
                        ) > len(reservations):
    ...
    def output_intervals(self, f3, merged_intervals, req_type, latency):
        ...
        weighted_bandwidth = self.flit_num * 128 / self.finish_time / self.config.num_ips
