...
class Feature_model(BaseModel):
    ...
    def update_intervals(self, flit, merged_intervals, latency, file, req_type):
        ...
        if flit.req_departure_cycle // self.config.network_frequency <= last_end:
            ...
            merged_intervals.append((flit.req_departure_cycle // self.config.network_frequency, flit.arrival_cycle // self.config.network_frequency, flit.burst_length))
...
def main():
    ...
    sim = Feature_model(config_path=config_path, topo_type=topo_type, traffic_file_path=traffic_file_path, file_name=file_name, result_save_path=result_save_path)
