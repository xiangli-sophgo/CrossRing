...
from src.traffic_process.traffic_gene.generation_engine import (
    ...
    generate_d2d_traffic_from_configs as generate_dcin_traffic_from_configs,
    ...
    for config_id, config in traffic_configs.items():
        ...
        if mode == "dcin":
            ...
            # 回退到旧格式
                ...
                raise ValueError(f"DCIN配置缺少DIE信息: {config_id}")
        else:
            # KCIN模式：一个配置生成一个引擎配置
            engine_config = {
                "src_map": src_map,
...
def _load_dcin_config_for_topology(topology: str) -> Optional[Dict]:
    """
    为指定拓扑加载DCIN配置
    ...
    Returns:
        DCIN配置字典，包含 num_dies 和 dcin_connections，如果未找到则返回None
    """
    # 尝试加载拓扑特定的DCIN配置
    config_file = TOPOLOGIES_DIR / f"dcin_{topology}_config.yaml"
    ...
    # 回退到默认配置
    default_config = TOPOLOGIES_DIR / "dcin_2die_config.yaml"
...
async def generate_traffic(request: TrafficGenerateRequest):
    ...
    try:
        ...
        if request.mode == "dcin":
            file_path, df = generate_dcin_traffic_from_configs(
        ...
        if request.split_by_source:
            ...
            if request.mode == "dcin":
                split_result = split_dcin_traffic_by_source(
