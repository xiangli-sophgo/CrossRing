...
interface MultiDieTopologyGraphProps {
  ...
  mounts: IPMount[]
  loading?: boolean
  // DCIN模式: {die_id: {link_key: bw}}
  linkBandwidth?: Record<string, Record<string, number>>
  // DCIN跨Die链路带宽: {'{src_die}-{src_node}-{dst_die}-{dst_node}': bw}
  dcinLinkBandwidth?: Record<string, number>
  linkComposition?: Record<string, any[]>
  dcinLayout: DCINLayoutInfo
...
const MultiDieTopologyGraph = forwardRef<{ saveLayout: () => void }, MultiDieTopologyGraphProps>(({
  ...
  // 使用useMemo同步计算elements，确保dcinLayout变化时立即更新
  const elements = useMemo(() => {
    if (!data || !dcinLayout) return []

    const { die_positions, die_rotations, dcin_connections, num_dies } = dcinLayout
    ...
    const computeDieOffsets = () => {
      ...
      dcin_connections.forEach(([srcDie, srcNode, dstDie, dstNode]) => {
    ...
    // 绘制DCIN跨Die连接线（双向）
    dcin_connections.forEach((conn, idx) => {
      ...
      const isForwardPositive = dcinDirection === 'horizontal'
        ? (dstX > srcX)
        : dcinDirection === 'vertical'
          ? (dstY > srcY)
          : (dstX > srcX)  // 对角线使用水平方向判断

      // 生成linkKey用于点击事件（带 dcin- 前缀）
      const fwdLinkKey = `dcin-${srcDie}-${srcNode}-${dstDie}-${dstNode}`
      const bwdLinkKey = `dcin-${dstDie}-${dstNode}-${srcDie}-${srcNode}`
      ...
      // 从后端获取DCIN链路带宽
      const fwdBandwidth = dcinLinkBandwidth?.[fwdBandwidthKey] || 0
      const bwdBandwidth = dcinLinkBandwidth?.[bwdBandwidthKey] || 0

      // 正向
      edges.push({
        data: {
          id: `dcin-conn-${idx}-fwd`,
          source: `die-${srcDie}-node-${srcNode}`,
          target: `die-${dstDie}-node-${dstNode}`,
          dcinConnection: true,
          direction: dcinDirection,
          offset: isForwardPositive ? 1 : -1,
  ...
  const handleCyInit = (cy: Cytoscape.Core) => {
    ...
    // DCIN跨Die链路点击事件
    cy.on('tap', 'edge.dcin-edge', (evt) => {
      const linkKey = evt.target.data('linkKey')
      if (linkKey && onLinkClick) {
        // 去除 "dcin-" 前缀用于查询 linkComposition
        const queryKey = linkKey.replace(/^dcin-/, '')
    ...
    <Card style={{ width: 'fit-content' }}>
      <Row style={{ marginBottom: 16 }} align="middle">
        <Col span={12}>
          <Space>
            <span>DCIN拓扑</span>
            <Tag color="blue">{dcinLayout.num_dies} Dies</Tag>
            <Tag color="cyan">{dcinLayout.dcin_connections.length} DCIN连接</Tag>
          </Space>
        </Col>
